...
function next(){
    ...
    if(batas_soal<=10){
        firebase.database().ref().child("soalkuis1.php").orderByChild("soalid").equalTo(emptyArray[indexx]).once("value", function(snapshot){
...
//fungsi menampilkan soal
        function tampil_soal_rand(){
            firebase.database().ref().child("soalkuis1.php").orderByChild("soalid").equalTo(randArray[panggil]).once("value", function(snapshot){
                snapshot.forEach(function(childSnapshot){
                    var soalid=childSnapshot.child("soalid").val();
                    var soal=childSnapshot.child("soal").val();
                    var opt1=childSnapshot.child("opt1").val();
